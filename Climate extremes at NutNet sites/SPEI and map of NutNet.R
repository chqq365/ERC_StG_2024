...
# check the intervals that climate extremes occur
dd1.inter<-dd1%>%filter(events!="Normal")%>%arrange(site_code, years)%>%group_by(site_code)%>%mutate(inter.l=years-lag(years))%>%
  mutate(inter.l1=ifelse(is.na(inter.l), 0, inter.l))
mean(dd1.inter$inter.l1)
...
###########################################################################################
#####################  world map indicating dry and wet sites #############################
###########################################################################################
drought.ind <- prec %>% merge(y = pet, by=c("site_code", "years", "months", "date")) %>%
  merge(y = month.df, by=c("months"))  %>%
  mutate(pet1 = pet * days) %>%
  filter(years>= 2000)%>%
  group_by(site_code, years)%>%
  summarise(sum.prec=sum(prec), sum.pet=sum(pet1))%>%mutate(pet_prec=sum.pet - sum.prec)%>%
  group_by(site_code)%>%summarise(Drought.index=mean(pet_prec))%>%
  merge(site.inf, by=c("site_code"))%>%
  mutate(experiment_type_1=ifelse(experiment_type=="Observational", "Observational sites", "Experimental sites"))
...
(pp.map<-ggplot(data = WorldData, aes(x = long, y = lat)) +
    ...
    geom_point(data = drought.ind, aes(x = longitude, y = latitude, color=Drought.index), alpha=0.5, size=6, position=position_jitter(h=0.5, w=0.5)) +
    scale_color_continuous(high = "#E69F00", low = "#0072B2") +
    ...
    theme(legend.position = c(0.05, 0.25), legend.background = element_blank()))
ggsave(pp.map, file="map for NutNet sites.png", height = 4.64, width = 13.3, dpi = 600)
